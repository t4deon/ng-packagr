...
class ResolvedDeclarationEmitter {
  ...
  emit(): string {
    ...
    for (const symbol of resolvedSymbols) {
      ...
      // upwards.
      while (!(decl.kind === ts.SyntaxKind.ExportKeyword) && decl.parent) {
        decl = decl.parent;
      }
      if (decl.kind === ts.SyntaxKind.ExportKeyword) {
            ...
            if (children.every(node => node.kind in memberDeclarationOrder)) {
              ...
              children.sort((a: ts.Declaration, b: ts.Declaration) => {
                ...
                return compareFunction(
                      a.kind === ts.SyntaxKind.StaticKeyword,
                      b.kind === ts.SyntaxKind.StaticKeyword) ||
                    // Our predefined order
                    compareFunction(
                      memberDeclarationOrder[a.kind],
                      memberDeclarationOrder[b.kind]) ||
                    // Alphebetical order
...
export const flattenDeclarationFiles = (entryFile: string): Promise<string> => {

    // TODO: ready entryFile (a .d.ts)

    // TODO: recursively collect all symbols

    // TODO: flatten to a single d.ts file
    ...
    // return Promise.resolve(publicApi(entryFile));
  };
